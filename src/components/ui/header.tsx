...
function NavLink({ href, children, className = '' }: { href: string; children: React.ReactNode; className?: string }) {
  return (
    <Link href={href.startsWith('https://recharm.com') ? href : `https://recharm.com${href}`} className={`inline-block rounded-lg py-1 px-2 text-sm text-slate-700 hover:bg-slate-100 hover:text-slate-900 ${className}`}>
      {children}
    </Link>
  )
}